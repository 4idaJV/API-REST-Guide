...
const signUp = async (req, res) => {
  try {
    ...
    return res.status(200).json({ token, role:"user", username: hola , id: id});  // ===> { token: token }
  } catch (error) {
...
const login = async (req, res) => {
  try {
    ...
    if (checkPass) {
      ...
      return res.status(200).json({ token, nombre:user.name , id:user.id }); // El objeto json contiene el token generado
    } else {
